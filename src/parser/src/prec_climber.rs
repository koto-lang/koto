// This is a clone of pest::PrecClimber, but with the addition of rules that can be skipped
...
impl<R: RuleType> PrecClimber<R> {
    ...
    /// ]);
    /// ```
    pub fn new(ops: Vec<Operator<R>>, skip_rules: Vec<R>) -> PrecClimber<R> {
        let ops = ops
            .into_iter()
            .zip(1..)
        ...
        PrecClimber {
            ops,
            skip_rules: skip_rules.iter().cloned().collect::<HashSet<_>>(),
        }
    }
            ...
            if let Some(&(prec, _)) = self.ops.get(&rule) {
                if prec >= min_prec {
                    ...
                    while pairs.peek().is_some() {
                        ...
                        } else if self.skip_rules.contains(&rule) {
                            // println!("Skipping: {:?}", &rule);
                            pairs.next().unwrap();
                            continue;
                        } else {
                            break;
