...
/// A newtype for plugin [KValue] that implements [Deserialize](serde_core::Deserialize).
#[derive(Clone)]
pub struct DeserializableKValue(pub KValue);

impl Default for DeserializableKValue {
    fn default() -> Self {
        Self(KValue::Null)
    }
}
...
impl<'de> Visitor<'de> for KValueVisitor {
        ...
        E: de::Error,
    {
        let data = v
            .iter()
            .map(|x| KValue::Number(KNumber::I64((*x).into())))
            .collect::<Vec<_>>();
        Ok(KValue::Tuple(<KTuple as KotoSequence<PluginBackend>>::from_slice(&data)).into())
    }
        ...
        A: de::SeqAccess<'de>,
    {
        ...
        Ok(DeserializableKValue(KValue::Tuple(
            <KTuple as KotoSequence<PluginBackend>>::from_slice(&data),
        )))
    }
    ...
    where
        A: de::MapAccess<'de>,
    {
        let mut data = Vec::with_capacity(map.size_hint().unwrap_or_default());
