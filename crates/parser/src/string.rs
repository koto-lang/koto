...
enum Inner {
    ...
    Slice(Ptr<StringSlice<usize>>),
    // A shared string with 32bit bounds, small enough to store without allocation
    Slice32(StringSlice<u32>),
}
...
impl KString {
    ...
    pub fn with_bounds(&self, new_bounds: Range<usize>) -> Option<Self> {
        match &self.0 {
            ...
            Inner::Slice32(slice) => slice.with_bounds(new_bounds).map(Self::from),
        }
    }
