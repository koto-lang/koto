...
fn format_node<'source>(
    ...
    match &node.node {
        ...
        Node::If(AstIf {
            ...
            // TODO: Get parser to emit block nodes for block expressions
...
impl<'source> FormatItem<'source> {
    ...
    fn render(&self, output: &mut String, options: &FormatOptions, column: usize) {
        match self {
            ...
            // Optional breaks are rendered as a space when the gruop fits within the remaining width
    ...
    fn is_break(&self) -> bool {
        match self {
            Self::LineBreak | Self::SpaceOrIndent | Self::ForceBreak | Self::MaybeIndent => true,
            _ => false,
        }
    }
}
